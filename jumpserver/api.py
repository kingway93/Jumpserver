...
def ldap_sync_list(ldapaddress, username, password, ou='People'):
    add_list = username.split(',')
    if "ou=" in username:
        del add_list[0]
    else:
        add_list[0] = 'ou=%s' %ou
    basedn = ','.join(add_list)
    ...
    try:
        ...
        while True:
            if result_data:
                user_dict = {}
                user_dict['username'] = result_data[0][0].split(',')[0].split('=')[1]
                user_dict['name'] = result_data[0][1]['cn'][0]
                user_dict['email'] = result_data[0][1]['mail'][0]
                user_list.append(user_dict)
                result_type, result_data = conn.result(ldap_result_id, 0)
            else:
                break
    except Exception:
        error = '连接失败，请检查用户名、密码和ldap服务器地址输入是否有误'
    return user_list, error

def ldap_login_auth(username, password, ldapaddr, ou, dc):
    dn = 'uid=%s,ou=%s,%s' %(username, ou, dc)
    my_ldap = ldap.open(ldapaddr)
    try:
