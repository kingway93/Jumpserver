...
@require_role(role='super')
def user_sync_list(request):
    error = ''
    msg = ''
    header_title, path1, path2 = '用户同步', '用户管理', '用户同步'
    if request.method == 'POST':
        ldapaddress = request.POST.get('ldapaddress','')
        username = request.POST.get('username','')
        password = request.POST.get('password','')
        ou = request.POST.get('ou','')
        if '' in [ldapaddress, username, password]:
            error = u'带*内容不能为空'
        else:
            try:
                user_list, error = ldap_sync_list(ldapaddress, username, password, ou='People')
                success_user_list = []
                new_dc = ','.join(username.split(',')[-2:])
                for user in user_list:
                    if User.objects.filter(username=user['username']):
                        pass
                    else:
                        uuid_r = uuid.uuid4().get_hex()
                        users = db_add_user(username=user['username'], name=user['name'],
                                       password='123456',email=user['email'],role='CU',
                                       uuid=uuid_r,groups = ['2'],admin_groups=[],is_active=1,
                                       date_joined=datetime.datetime.now(),is_ldap=1,ou=ou,
                                       dc=new_dc,ldapaddr=ldapaddress)
                        server_add_user(username=user['username'])
                        success_user_list.append(user['username'])
                        user = get_object(User,username=user['username'])
                msg = u'用户同步成功,同步成功的用户有: %s' %(', '.join(success_user_list))
                if success_user_list:
                    logger.debug(u'ldap同步的用户有: %s' %(', '.join(success_user_list)))
            except Exception,e:
                error = u'用户同步失败: %s'%(e)
    return my_render('juser/user_sync_list.html',locals(),request)
